...
import { getSandbox, parseAgentOutput, lastAssistantTextMessageContent, logAssistantTexts, azureOpenAICompat, normalizeSandboxPath, type AzureCompatOptions } from "@/inngest/utils";
...
async function getOrCreateSandboxId(step: any) {
  return await step.run("get-sandbox-id", async () => {
    ...
    try {
      ...
      console.error("❌ Failed to create E2B sandbox:", err?.message);
      throw err;
    }
  });
}
...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    console.log("🔧 Starting code-agent function");
                ...
                for (const file of files) {
                  ...
                  if (/^app\/page\.(tsx|jsx|ts|js)$/i.test(normalized)) {
                    ...
                    if (shouldBlockDestructivePageWrite(prev || "", file.content)) {
                      console.warn("🚫 Blocked destructive overwrite of app/page.*");
                  ...
                  if (/\/demo\.(tsx|jsx|ts|js)$/i.test(normalized)) {
                    console.warn(`🚫 Demo files not allowed: ${normalized}`);
                    continue;
                  }
                  if (/app\/page\.(tsx|jsx|ts|js)$/i.test(normalized) && DEFAULT_NEXT_TEMPLATE_RE.test(file.content)) {
                    console.warn("🚫 Skipping Next.js starter content overwrite");
                    continue;
                  }
          ...
          handler: async ({ files }, { step, network }) => {
            ...
            return await step?.run("readFiles", async () => {
              try {
                ...
                console.log("✅ Read", contents.length, "files successfully");
    ...
    const network = createNetwork<AgentState>({
      ...
      router: async ({ network }) => {
        ...
        if (summary) {
          console.log("🏁 Task completed, ending");
          return;
        }
    ...
    console.log("📋 Task steps generated");
    ...
    await step.run("save-steps", async () => {
      ...
      console.log("💾 Steps saved:", stepsMessage.id);
      return stepsMessage;
    });

    const result = await network.run(enrichedInstruction, { state });
    console.log("🎯 Network execution", result.state.data.summary ? "completed" : "failed");
  ...
  logAssistantTexts("fragment-title-generator", fragmentTitleOutput);
    console.log("🏷️ Fragment title generated");
      ...
      await step.run("inject-analytics", async () => {
        try {
          ...
          if (layoutPath && layoutRaw) {
            ...
            console.log("✅ Analytics injected via layout");
            return;
          }
          ...
          if (targetPath && updatedContent) {
            ...
            console.log("✅ Analytics injected via head fallback");
          }
        } catch (err) {
  ...
  const successMessage = await prisma.message.create({
      ...
      console.log("✅ Saved success result:", successMessage.id);
      return successMessage;
    });
    console.log("🎉 Code-agent function completed");
    return {
      url: sandboxUrl,
      title: "Fragment",
      files: result.state.data.files,
...
export const codeAgentEditFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    console.log("🔧 Starting code-agent-edit");
    const sandboxId = await getOrCreateSandboxId(step);
  ...
  normalizedAllowed.add(hasSrc ? "src/app/*" : "app/*");
    ...
    logAssistantTexts("task-steps-generator", taskStepsOutput);
    console.log("📋 Task steps generated for edit");
    ...
    await step.run("save-steps-for-edit", async () => {
      ...
      console.log("💾 Edit steps saved:", stepsMessage.id);
      return stepsMessage;
    });
